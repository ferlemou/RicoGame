...
public class CapivaraTrem : MonoBehaviour
{
    ...
    private void OnCollisionEnter2D(Collision2D other)
    {
        ...
        /*
        if (other.gameObject.CompareTag("Obstacle") || other.gameObject.CompareTag("TopObstacle") && invisibility)
        {
            Debug.Log("ignorou colisao");
            BoxCollider2D collider = other.collider as BoxCollider2D;
            Physics2D.IgnoreCollision(playerCollider, collider);
        }
        if (other.gameObject.CompareTag("Obstacle") && !invisibility)
        {
            Morreu();
        }
        if (other.gameObject.CompareTag("TopObstacle") && !invisibility)
        {
            chao = true;
        }
        */
    ...
    public void Jump()
    {
        if (canJump && chao && Vivo && !hyperJump)
        {
            rig.AddForce(Vector2.up * 0, ForceMode2D.Impulse);
        ...
        else if (canJump && chao && Vivo && hyperJump)
        {
            //Debug.Log("deu super salto");
            rig.AddForce(Vector2.up * 0, ForceMode2D.Impulse);
